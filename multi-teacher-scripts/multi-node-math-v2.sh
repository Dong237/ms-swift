...
#       mixed-teacher batches — the high-KL math tokens dominate the gradient
...
#           loss = (loss_math + loss_anchor_memory) / 2
...
#           [Step 42] Routing: anchor_memory=4, math=4
...
#          to stay close to teacher distribution; math uses vanilla Qwen3-8B,
#          reduce beta so student can deviate more from the prior.
# TEACHER_BETA_MAP='{"math": 0.7, "anchor_memory": 0.95}'
# TEACHER_TEMPERATURE_MAP='{"math": 1.0, "anchor_memory": 0.8}'

run_name="Qwen3-1.7B-multi-teacher-gkd-math-8B-anchor-8b-200700-v2"
...
# stderr.log  — domain loss summary ([Step N] domain_loss/math = ...)
...
swift rlhf \
    ...
    --dataset anchor_memory_policy_sft200700-ms-swift competition_math_gkd \
